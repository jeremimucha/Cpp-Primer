...
map<const string, string> buildMap(ifstream& map_file);
const string& transform(const string& s, const map<const string, string>& m);
...
void word_transform(ifstream& map_file, ifstream& input_file)
{
    map<const string, string> trans_map = buildMap(map_file);
    // std::cerr << "The map:\n";
...
map<const string, string> buildMap(ifstream& map_file)
{
    map<const string, string> trans_map;
    string key;
    string value;
...
const string& transform(const string& s, const map<const string, string>& m)
{
    ...
    map<const string, string>::const_iterator map_it = m.find(s);
